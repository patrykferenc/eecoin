...
func (h *acceptClientMessageHandler) Handle(cmd AcceptClientMessage) error {
	...
	data := node.SendMessageEvent{TransactionID: cmd.Transaction().ID}
	event, err := event.New(data, "x.message.send")
	if err != nil {
	...
	slog.Info("Accepted client message", "transactionID", cmd.Transaction().ID, "content", cmd.Transaction().Content)
